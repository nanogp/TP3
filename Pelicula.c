...
void eMovie_initHardcode(eMovie* listadoPeliculas)
{
    ...
    for(int i = 0; i<3 ; i++)
    {
        strcpy(&((listadoPeliculas+i)->pais), (paises+i));
        strcpy(&((listadoPeliculas+i)->nombre), (nombres+i));
        (listadoPeliculas+i)->dni = *(dni+i);
        (listadoPeliculas+i)->reservas = *(reservas+i);
...
int eMovie_mostrarUno(eMovie* pelicula)
{
     ...
     {
        retorno = 0;
        printf(PELICULA_MOSTRAR_UNO_MASCARA, pelicula->idPelicula, pelicula->titulo, pelicula->genero, pelicula->puntaje, pelicula->duracion, pelicula->descripcion);
     }
     return retorno;
}
...
int eMovie_modificacion(eMovie* listadoPeliculas, int limitePeliculas)
{
    ...
    imprimirTitulo(PELICULA_MODIFICACION_TITULO);
    ...
    else
    {
        ...
        {
            listadoPeliculas[posicion] = registro;
            eMovie_ordenar(listadoPeliculas, limitePeliculas, PELICULA_ORDEN_ID);
...
int eMovie_baja(eMovie* listadoPeliculas, int limitePeliculas)
{
    ...
    ejecutarEnConsola(LIMPIAR_PANTALLA);
    imprimirTitulo(PELICULA_BAJA_TITULO);
    ...
    if(listadoPeliculas != NULL && limitePeliculas > 0)
    {
        ...
        else if(strcmp(orden, "nombreAsc") == 0)
        {
            for(i=0 ; i<limitePeliculas-1 ; i++)
            {
                for(j=i+1 ; j<limitePeliculas ; j++)
                {
                    if(strcmpi((listadoPeliculas+i)->nombre, (listadoPeliculas+j)->nombre) > 0)
                    {
                        aux = *(listadoPeliculas+i);
                        *(listadoPeliculas+i) = *(listadoPeliculas+j);
                        *(listadoPeliculas+j) = aux;
        ...
        else if(strcmp(orden, "kAsc") == 0)
        {
            for(i=0 ; i<limitePeliculas-1 ; i++)
            {
                for(j=i+1 ; j<limitePeliculas ; j++)
                {
                    if(strcmpi((listadoPeliculas+i)->reservas, (listadoPeliculas+j)->reservas) > 0)
                    {
                        aux = *(listadoPeliculas+i);
                        *(listadoPeliculas+i) = *(listadoPeliculas+j);
                        *(listadoPeliculas+j) = aux;
